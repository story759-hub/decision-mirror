...
export default function ClarityRoom() {
  ...
  const handleSaveImage = async () => {
    ...
    try {
      // 캡처 시 레이아웃 깨짐 방지를 위한 옵션 설정
      const canvas = await html2canvas(cardRef.current, {
        scale: 2,             // 고해상도 저장
        useCORS: true,        // 외부 리소스 허용
        backgroundColor: null, // 투명 배경 유지 (카드 자체 배경 사용)
        logging: false,
      });
      ...
      <main className="max-w-lg mx-auto px-6">
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-10 duration-1000">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <button onClick={handleSaveImage} className="py-5 bg-white/5 text-slate-300 rounded-2xl font-bold text-sm border border-white/10 active:bg-white/10">이미지 저장 💾</button>
                <button onClick={handleShare} className="py-5 bg-white/5 text-slate-300 rounded-2xl font-bold text-sm border border-white/10 active:bg-white/10">결과 공유 🔗</button>
              </div>
              {!data.isTrivial && (
                <button onClick={handleDeepAnalyze} className="w-full py-5 bg-[#5D5FEF] text-white rounded-2xl font-black text-sm shadow-xl active:scale-95 transition-all">심층 분석 (30초 정제) 🔓</button>
              )}
              <button onClick={() => { setStage('input'); setInput(''); }} className="w-full py-4 text-slate-500 font-bold text-xs uppercase tracking-widest">New Entry</button>
            </div>
          </div>
        )}
        ...
        {stage === 'deep_result' && data && (
          <div className="space-y-8 animate-in zoom-in-95 duration-700">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <button onClick={handleSaveImage} className="py-5 bg-white/10 text-white rounded-2xl font-bold text-sm border border-white/20 active:bg-white/20">이미지 저장 💾</button>
                <button onClick={handleShare} className="py-5 bg-white/10 text-white rounded-2xl font-bold text-sm border border-white/20 active:bg-white/20">결과 공유 🔗</button>
              </div>
