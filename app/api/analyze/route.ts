...
[지시사항]
1. 분석 내용 반영: 사용자의 메시지 맥락을 깊이 읽고 공감하는 내용을 "description"의 첫 줄에 작성하세요. 메시지가 없거나 부적절한 경우 선택된 감정과 태그를 바탕으로 보편적인 위로를 작성합니다.
2. 금지 사항: 성적, 폭력적, 부적절한 언어는 절대 사용하지 마세요.
3. 규격 준수: "description"은 반드시 공백 포함 75자 미만으로 작성하며, 가독성을 위해 중간에 '실제 줄바꿈'을 한 번 포함하여 두 줄로 구성하세요. (주의: \n 이라는 텍스트 문자를 직접 출력하지 말고, 실제 엔터 키를 친 것과 같은 줄바꿈 결과만 전달할 것)
4. 추천곡: 감정에 어울리는 실제 노래를 "가수 - 제목" 형식으로 "song" 필드에 작성하세요.
5. 감정 비율: 'mix' 필드의 'key'값은 감정의 핵심을 담은 단어로 구성하되, 최대 4글자(한글/영문/이모지 포함) 이내로 자유롭게 작성하세요. (예: [몽글몽글, 뿌듯, 약간지침, 🔥🔥🔥]) 전체 비율 합은 100이 되어야 합니다.      
      결과 형식(JSON):
      { 
        "description": "문장", 
        "song": "가수 - 제목", 
