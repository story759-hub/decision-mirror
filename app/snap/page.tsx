...
type Stage = 'pick' | 'resonate' | 'deep' | 'analyzing' | 'result' | 'archive';
...
export default function FeelingSnapFinal() {
  ...
  const fetchHistory = async () => {
    ...
    if (!fp) return;
    try {
      const res = await fetch(`/api/analyze?fp=${fp}&t=${Date.now()}`);
  ...
  const handleViewArchive = (item: any) => {
    ...
    const archivedResult = {
      ...
      song: item.song,
      mix: (Array.isArray(item.mix_data) ? item.mix_data : []).map((m: any) => ({
        ...m,
        label: m.label || EMOTION_DATA[m.key]?.label || "기록",
      })), 
      mainEmotion: EMOTION_DATA[item.emotion_key] || EMOTION_DATA.neutral,
      subName: item.reason,
      displayStats: {
        userSnapCount: 'History',
        totalCount: "ARCHIVED",
  ...
  const handleFinalAnalyze = async () => {
    ...
    try {
      ...
      if (!response.ok || aiData.error) throw new Error(aiData.message || "분석 실패");

      const updatedHistory = await fetchHistory();
      const todayStr = new Date().toLocaleDateString();
      const todayCount = (updatedHistory || []).filter((item: any) => 
        new Date(item.created_at).toLocaleDateString() === todayStr
      ).length;
      ...
      setTimeout(() => {
        const finalData = {
          ...
          displayStats: {
            ...aiData.displayStats,
            userSnapCount: updatedHistory?.length || aiData.displayStats?.userSnapCount,
            emotionSpecificCount: todayCount
          }
        };
  ...
  const handleShare = async () => {
    ...
    } catch (err) { alert("공유 중 오류가 발생했습니다."); }
  };
        ...
        {history.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4">
            {displayHistory.map((item) => (
              <ArchiveCard key={item.id} item={item} onClick={() => handleViewArchive(item)} />
            ))}
          </div>
        )}
      </div>
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 pb-10 overflow-x-hidden font-sans flex flex-col">
      {/* ✅ 상단 내비게이션 바: 메인으로 가기 버튼 추가 */}
      <nav className="max-w-md mx-auto w-full px-6 pt-6 flex justify-between items-center">
        <Link href="/" className="group flex items-center gap-2 text-[11px] font-bold text-slate-400 hover:text-slate-900 transition-colors">
          <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" /> 
          메인 홈페이지
        </Link>
        {stage !== 'pick' && (
          <button onClick={() => { setStage('pick'); setTextInput(''); }} className="text-[11px] font-bold text-slate-300 hover:text-[#E91E63]">
            다시 찍기
          </button>
        )}
      </nav>
      ...
      <main className="max-w-md mx-auto px-6 flex-grow">
        {stage === 'pick' && (
        ...
        {stage === 'archive' && (
          <div className="animate-in slide-in-from-right-4 duration-500 pb-20">
            <div className="flex items-center gap-4 mb-8">
              <button onClick={() => setStage('pick')} className="p-2 hover:bg-slate-100 rounded-full transition-colors">
                <ArrowLeft size={20} />
              </button>
              <h2 className="text-2xl font-black text-slate-800 tracking-tighter">전체 기록 보관함</h2>
            </div>
            ...
            <div className="p-6 bg-slate-50 rounded-[32px] text-center">
              <p className="text-[12px] text-slate-400 font-medium leading-relaxed">
                무료 버전에서는 최신 10개까지만 보관됩니다.<br/>
                <span className="text-[#E91E63] font-bold">PRO 버전</span> 출시 시 1개월 무제한 보관 기능을 만나보세요.
              </p>
            </div>
          </div>
        )}
                    ...
                    <div className="flex items-center justify-end gap-3 group cursor-pointer" onClick={() => openYouTubeSearch(resultData.song)}>
                      <div className="text-right max-w-[140px]">
                        ...
                        <p className={`${myeongjo.className} text-[11px] sm:text-[12px] font-bold text-white/40 group-hover:text-[#E91E63] transition-colors truncate italic`}>
                          {resultData.song?.includes?.(' - ') ? resultData.song.split(' - ')[1] : (resultData.song || 'Untitled')}
                        </p>
                      </div>
